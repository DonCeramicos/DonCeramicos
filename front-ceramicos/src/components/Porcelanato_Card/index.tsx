"use client";
import { Iporcelanatos } from "@/types";
import Image from "next/image";
import { Eye, EyeClosed } from "lucide-react";
import { useState } from "react";

type PorcelanatoCardProps = {
  porcelanato: Iporcelanatos;
  onClick?: () => void;
  isInOffersSection?: boolean;
};

export const Porcelanato_Card = ({
  porcelanato,
  onClick,
  isInOffersSection = false,
}: PorcelanatoCardProps) => {
  ...
  return (
    ...
    >
      {isInOffersSection && porcelanato.oferta && (
      ...
      <Image
        src={porcelanato.imagen?.[0] || "/placeholder.png"}
        alt={porcelanato.nombre}
        width={500}
        height={500}
      ...
      <div className="p-1 flex flex-col items-center text-[13px] w-full text-[#c0b283] font-phudu mt-2 tracking-wide">
        <p className="color-font-2">{porcelanato.nombre}</p>
      </div>
